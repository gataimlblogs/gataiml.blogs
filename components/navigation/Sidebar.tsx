  ...
  useEffect(() => {
    ...
    if (user) {
      ...
      if (user.role === 'FACULTY') {
        ...
        setPendingVerifications(verifications.filter(v => v.status === 'PENDING').length);
      }
    }
  }, []);
    ...
    <aside className="w-full flex flex-col gap-4 animate-fade-in">
      ...
      <div className="rounded-2xl border border-[var(--border-color)] bg-[var(--bg-secondary)] overflow-hidden shadow-sm hover:shadow-md transition-all duration-300">
        ...
        <div className="px-4 pb-4 relative">
          ...
          <div className="pt-10">
            <Link href={`/profile/${currentUser.username}`} className="block group">
            ...
            <p className="text-xs text-[var(--text-muted)] font-medium">@{currentUser.username}</p>
            ...
            {currentUser.role === 'STUDENT' && (
              <>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="line-clamp-1">{profile?.department || 'CSE Department'}</span>
                </div>
              </>
            )}
              ...
              <>
                <div className="flex items-center gap-2">
                  ...
                  <span className="line-clamp-1 font-semibold text-[var(--text-primary)]">
                    {profile?.jobTitle || 'Alumnus'} {profile?.company ? `at ${profile.company}` : ''}
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  <GraduationCap className="w-4 h-4 text-[var(--text-muted)]" />
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span>
                    Status: {currentUser.isVerified ? (
                      <span className="text-indigo-600 dark:text-indigo-400 font-bold">Verified Alumnus</span>
                    ) : (
                      <span className="text-amber-600 dark:text-amber-400 font-bold">Pending Verification</span>
                    )}
                  </span>
                </div>
              </>
            ...
            {currentUser.role === 'FACULTY' && (
              <>
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-bold text-rose-600 dark:text-rose-400">Institutional Faculty Moderator</span>
                </div>
                <div className="flex items-center gap-2">
                  <Building className="w-4 h-4 text-[var(--text-muted)]" />
        ...
        <div className="bg-[var(--bg-tertiary)] px-4 py-3 flex items-center justify-between text-xs text-[var(--text-secondary)] border-t border-[var(--border-color)]">
          <div className="flex flex-col">
            <span className="font-bold text-[var(--text-primary)]">{postsCount}</span>
            <span className="text-[10px] text-[var(--text-muted)]">My Posts</span>
          </div>
          {currentUser.role === 'FACULTY' ? (
            <Link href="/faculty" className="flex flex-col items-end hover:underline">
              <span className="font-bold text-rose-600 dark:text-rose-400">{pendingVerifications}</span>
              <span className="text-[10px] text-[var(--text-muted)]">Pending Approvals</span>
            </Link>
          ) : (
            <Link href="/directory" className="flex flex-col items-end hover:underline">
              <span className="font-bold text-[var(--text-primary)]">64+</span>
              <span className="text-[10px] text-[var(--text-muted)]">College Contacts</span>
            </Link>
          )}
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-[var(--border-color)] bg-[var(--bg-secondary)] p-4 shadow-sm">
        <h4 className="font-bold text-sm text-[var(--text-primary)] mb-3">Quick Actions</h4>
