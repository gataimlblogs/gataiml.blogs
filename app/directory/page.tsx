...
export default function DirectoryPage() {
  ...
  const [filterRoles, setFilterRoles] = useState<Set<Role>>(
    new Set(["STUDENT", "ALUMNI", "FACULTY"] as const),
  );
                      ...
                      {profile?.skills && profile.skills.length > 0 && (
                        <div className="flex flex-wrap gap-1 mt-3">
                          {profile.skills.slice(0, 3).map((skill) => (
                            <span
                              key={skill}
                              className="px-1.5 py-0.5 rounded bg-[var(--bg-tertiary)] border border-[var(--border-color)] text-[9px] font-semibold text-[var(--text-secondary)]"
                            >
                              {skill}
                            </span>
                          ))}
