...
import { Newspaper, Calendar, GraduationCap, Flame, ArrowRight, ExternalLink } from 'lucide-react';
...
export default function HomeFeed() {
  ...
  const featuredAlumni = mockStore.getUsers()
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-6 flex flex-col gap-5">
          ...
          <div className="flex items-center justify-between px-1">
            <h3 className="font-extrabold text-base text-[var(--text-primary)]">Activity Feed</h3>
            <span className="text-xs text-[var(--text-muted)] font-medium">Sorted by: Newest</span>
          </div>
          ...
          <div className="rounded-2xl border border-[var(--border-color)] bg-[var(--bg-secondary)] p-4 shadow-sm">
            ...
            <div className="flex flex-col gap-3.5">
              ...
              <div className="group cursor-pointer">
                <span className="text-[10px] font-bold text-[var(--accent-primary)] uppercase tracking-wider block">Placement Update</span>
                <p className="text-xs font-bold text-[var(--text-primary)] group-hover:text-[var(--accent-primary)] line-clamp-2 mt-0.5 leading-snug transition-colors">
                  Highest package touches record ₹48 LPA at CSE placements this season
                </p>
                <span className="text-[9px] text-[var(--text-muted)] block mt-1">126 students applied • 2d ago</span>
              </div>

              <div className="group cursor-pointer">
                <span className="text-[10px] font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-wider block">Hackathon</span>
                <p className="text-xs font-bold text-[var(--text-primary)] group-hover:text-[var(--accent-primary)] line-clamp-2 mt-0.5 leading-snug transition-colors">
                  Registration open for Annual Innowave 48-Hour Hackathon 2026
                </p>
                <span className="text-[9px] text-[var(--text-muted)] block mt-1">45 teams registered • 4d ago</span>
              </div>

              <div className="group cursor-pointer">
                <span className="text-[10px] font-bold text-rose-600 dark:text-rose-400 uppercase tracking-wider block">Campus Event</span>
                <p className="text-xs font-bold text-[var(--text-primary)] group-hover:text-[var(--accent-primary)] line-clamp-2 mt-0.5 leading-snug transition-colors">
                  Distinguished Speaker Lecture on Quantum Computing by ISRO Scientists
                </p>
                <span className="text-[9px] text-[var(--text-muted)] block mt-1">Seminar Hall 1 • 6d ago</span>
              </div>
            ...
            <div className="flex flex-col gap-3">
              {featuredAlumni.map((alumnus) => (
                <div key={alumnus.id} className="flex gap-2.5 items-start">
                  <Link href={`/profile/${alumnus.username}`} className="btn-interactive flex-shrink-0 block">
                  ...
                  <div className="flex-grow">
                    <Link href={`/profile/${alumnus.username}`} className="hover:underline">
                      <div className="text-xs font-bold text-[var(--text-primary)] leading-tight">{alumnus.name}</div>
                    </Link>
                    <span className="text-[9px] text-[var(--text-muted)] block mt-0.5">@{alumnus.username}</span>
