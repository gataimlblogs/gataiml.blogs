...
export default function FacultyPage() {
    ...
    return (
      <div className="max-w-xl mx-auto px-4 py-20 text-center">
        ...
        <p className="text-sm text-[var(--text-secondary)] mt-2 max-w-sm mx-auto leading-relaxed">
          The Faculty Moderation Board is only accessible to verified faculty
          members.
        </p>
        <button
          onClick={() => router.push("/")}
  ...
  const handleReview = (requestId: string, status: VerificationStatus) => {
    ...
    setTimeout(() => {
      mockStore.reviewVerificationRequest(
        requestId,
        currentUser.id,
        status,
    ...
    <div className="max-w-4xl mx-auto px-4 py-6 pb-24 md:pb-8">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        {[
          ...
          <button
            key={key}
            onClick={() => setFilter(key as "ALL" | VerificationStatus)}
      ...
      {filteredRequests.length === 0 ? (
        <div className="rounded-2xl border border-[var(--border-color)] bg-[var(--bg-secondary)] p-12 text-center">
          ...
          <p className="text-xs text-[var(--text-muted)] mt-1">
            No {filter !== "ALL" ? filter.toLowerCase() : ""} verification
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <div className="flex items-center gap-2 flex-wrap">
                        <Link
                          href={`/profile/${req.alumniUsername}`}
                      ...
                      <div className="flex items-center gap-3 mt-1 text-[10px] text-[var(--text-muted)]">
                        ...
                        <span>·</span>
                        <span>
                          Requested {getRelativeTimeIST(req.createdAt)}
                        </span>
                      </div>
                    </div>
                  </div>
                  <ChevronDown
                    className={`w-4 h-4 text-[var(--text-muted)] flex-shrink-0 transition-transform ${isExpanded ? "rotate-180" : ""}`}
                  />
                </button>
